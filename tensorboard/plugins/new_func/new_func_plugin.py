...
from tensorboard.plugins.new_func import metadata
from tensorboard.compat import tf
from tensorboard.plugins.new_func.new_func_loader import train
import os
...
class NewFuncPlugin(base_plugin.TBPlugin):
  ...
  def get_plugin_apps(self):
    return {
        ...
        '/trainprocess':self._serve_train_process,
  ...
  def frontend_metadata(self):
    return super(NewFuncPlugin, self).frontend_metadata()._replace(
        element_name='tf-new_func-dashboard',
    )
  ...
  def _serve_train_process(self, request):
    ...
    if result=="success!":
      ...
      return http_util.Respond(request, {'ifAccomplished' : False}, 'application/json')
